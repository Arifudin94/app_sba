...
<?php
	   ...
	   echo '<div class="col-sm-6">
			<table>';
			   ...
			   echo '<h6 style="text-align:justify;" id="alamat2">Dengan ini 
					 <b id="alamat3">PIHAK PERTAMA</b> menyerahkan kepada 
					 <b id="alamat3">PIHAK KEDUA</b> perangkat-perangkat</h6>';
			   echo '<h6 id="alamat2">IT tersebut dibawah ini :</h6>';
   ...
   $qhp_pj = mysql_query("SELECT merek,type,sn,no_inventory,keterangan FROM tbl_headset_pj WHERE nis='$nis'");
   if(mysql_num_rows($qhp_pj) > 0){
      $no = 1;
      while(list($merek,$type,$sn,$no_inventory,$keterangan) = mysql_fetch_array($qhp_pj)){
         echo '<tr><td>'.$no.'</td>';
         echo '<td>'.$merek.'</td>';
         echo '<td>'.$type.'</td>';
         echo '<td>'.$sn.'</td>';
