...
if( empty( $_SESSION['iduser'] ) ){
	...
	if( isset( $_REQUEST['submit'] )){
		$nis = $_REQUEST['nis'];
		$sql = mysql_query("DELETE FROM tbl_headset_hl WHERE nis='$nis'");
		if($sql > 0){
      ...
      echo '<div class="row">';
		...
		echo '<br>Type   : '.$type;
		echo '<br>';
		echo '<a href="./admin.php?hlm=master&sub=hl_headset&aksi=delete&submit=ya&nis='.$nis.'" class="btn btn-sm btn-success">Ya, Hapus</a> ';
		echo '<a href="./admin.php?hlm=master&sub=hl_headset" class="btn btn-sm btn-default">Tidak</a>';
		echo '</div>';
	}
}
