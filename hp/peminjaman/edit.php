...
if( empty( $_SESSION['iduser'] ) ){
	...
	if( isset( $_REQUEST['submit'] )){
		...
		$sql = mysql_query("UPDATE tbl_hp_pj SET nama='$nama', merek='$merek', jabatan='$jabatan', satuan_kerja='$satuan_kerja', tgl_surat='$tgl_surat',
		no_surat='$no_surat', it='$it', type='$type', sn='$sn', no_inventory='$no_inventory', keterangan='$keterangan' WHERE nis='$nis'");
		
		if($sql > 0){
			$sql = mysql_query("SELECT * FROM tbl_hp_pj ORDER BY nis");
		echo '<h2>Data Peminjaman HP</h2><hr>';
      echo '<div class="row">';
		...
		echo '<tr class="info" >
				...
				<th>Keterangan</th>
				';
		echo '<th width="85"><a href="./admin.php?hlm=master&sub=pj_hp&aksi=add"><img src="add-file.png" width="20" height="20" /></a></th></tr>';
		...
		if( mysql_num_rows($sql) > 0 ){
			...
			while(list($nis,$nama,$jabatan,$satuan_kerja,$tgl_surat,$it,$no_surat,$merek,$type,$sn,$no_inventory,$keterangan) = mysql_fetch_array($sql)){
				...
				echo '<td><a href="./admin.php?hlm=master&sub=pj_hp&aksi=edit&nis='.$nis.'"><img src="writing.png" width="20" height="20" /></a> ';
				echo '<a href="./admin.php?hlm=master&sub=pj_hp&aksi=print&nis='.$nis.'"><img src="printer-with-paper.png" width="20" height="20" /></a> ';
				echo '<a href="./admin.php?hlm=master&sub=pj_hp&aksi=delete&nis='.$nis.'"><img src="recycle-bin.png" width="20" height="20" /></a></td>';
				echo '</tr>';
				$no++;
			}
		} else {
		...
		$nis = $_REQUEST['nis'];
		$sql = mysql_query("SELECT * FROM tbl_hp_pj WHERE nis='$nis'");
		list($nis,$nama,$jabatan,$satuan_kerja,$tgl_surat,$no_surat,$it,$merek,$type,$sn,$no_inventory,$keterangan) = mysql_fetch_array($sql);
?>
<h2>Edit Form Surat BA Peminjaman HP</h2>
<hr>
<form method="post" action="admin.php?hlm=master&sub=pj_hp&aksi=edit" class="form-horizontal" role="form">
	...
	<div class="form-group">
		<div class="col-sm-offset-2 col-sm-10">
			...
			<a href="./admin.php?hlm=master&sub=pj_hp" class="btn btn-link">Batal</a>
		</div>
	</div>
</form>
<?php
