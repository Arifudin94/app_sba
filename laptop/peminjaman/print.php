...
<?php
   ...
   $qlaptop_pj = mysql_query("SELECT merek,type,sn,no_inventory,keterangan FROM tbl_laptop_pj WHERE nis='$nis'");
   if(mysql_num_rows($qlaptop_pj) > 0){
      $no = 1;
      while(list($merek,$type,$sn,$no_inventory,$keterangan) = mysql_fetch_array($qlaptop_pj)){
         echo '<tr><td>'.$no.'</td>';
         echo '<td>'.$merek.'</td>';
         echo '<td>'.$type.'</td>';
         echo '<td>'.$sn.'</td>';
